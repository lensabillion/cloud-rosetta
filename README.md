...
| ❌ | **How a permission decision is reached** | [Explicit deny, then SCP, then resource policy, then boundary, then session, then identity policy](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_evaluation-logic.html) | [Deny assignment, then additive union of role assignments](https://learn.microsoft.com/en-us/azure/role-based-access-control/overview) | [Deny policy, then additive union of allow policy bindings](https://docs.cloud.google.com/iam/docs/deny-overview) |
| ❌ | **Administering the directory versus administering resources** | *none* | [Microsoft Entra roles, separate from Azure RBAC roles](https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/custom-overview) | *none* |
| ❌ | **Instance-level traffic filtering** | [Security group](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-security-groups.html) | [Network security group](https://learn.microsoft.com/en-us/azure/virtual-network/network-security-groups-overview) | [VPC firewall rule](https://docs.cloud.google.com/firewall/docs/firewalls) |
...
- **High availability versus read scaling on a relational database.** The classic RDS trap, and it transfers.
...
- **Grouping container below the isolation boundary.** Two clouds use the identical phrase for unrelated things and the third has neither.
...
- **How a permission decision is reached.** AWS layers six evaluation stages and a permission must survive all of them, with any explicit deny final.
- **Administering the directory versus administering resources.** Azure alone runs two independent role systems.
- **Instance-level traffic filtering.** An AWS security group cannot express a deny rule.
...
- **Naming the replication and durability setting.** Azure exposes redundancy as an explicit, separately chosen four-letter setting on the storage account, and AZ-104 tests the letters directly: LRS keeps three copies in one datacentre, ZRS spreads across zones, GRS adds a second region asynchronously, GZRS combines both, and the RA- prefix makes the secondary readable.
