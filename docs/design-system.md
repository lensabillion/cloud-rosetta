...
4. **Four type sizes, and 16px is the floor for anything a reader must read.** Labels may go to
   13px. Nothing goes below that, drawings included: every label in every figure is now 13px,
   so the earlier 11px exception has been retired.
...
Generated by `scripts/diagrams.py` and placed in a chapter with a comment such as
`<!-- diagram: hierarchy -->`. Each exists to show a mechanism the prose cannot.

**15 definitions render 24 figures** across the guide, covering the resource hierarchy, scope,
identity, firewalling, private access, database failover, per-cloud application views and the
deployment architectures.

Every figure is also written as a standalone file under `site/assets/architecture/`, linked from
its own caption as "Open full-size SVG", so a drawing can be opened on its own or shared without
the page around it.
...
- **Labels anchor to the top edge of a box**, never the middle. Centred labels collide the
  moment boxes nest, which is what happened on the first attempt.
- **All drawn text is 13px**, matching the interface floor.
- The architecture chapters follow the [C4 notation guidance](https://c4model.com/diagrams/notation)
  on titles, scope, element labels and directional relationships, with an explicit legend, while
  stating plainly that they are conceptual views for learning rather than deployment plans.

**Still not automated:** no build-time check that two `<text>` bounding boxes in a figure never
overlap. It is run by hand and currently passes across all 24 figures and 541 labels, but nothing
would catch a regression. Roughly fifteen lines of work.

## Measured State, 15 September 2026

Re-measured at a 1280px viewport after the architecture and navigation work.

| Criterion | Target | Measured |
| --- | --- | --- |
| Reading column | 640px or less | 620px |
| Characters per line | 60 to 75 | 64 to 73, median 69 |
| Distinct interface font sizes | fewer than 6 | 5, at 13, 14, 16, 23 and 34px |
| Smallest text, drawings included | 13px floor | 13px |
| Sidebar links landing on a real page | all | 16 of 16 |
| Broken in-page links | none | none |
| Overlapping labels across 24 figures | none | none |
